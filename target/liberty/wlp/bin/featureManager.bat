...
if exist "%JAVA_HOME%\jmods\java.base.jmod" set JVM_ARGS=--add-opens java.base/java.lang=ALL-UNNAMED --add-exports java.base/sun.security.action=ALL-UNNAMED !JVM_ARGS!

set JVM_ARGS=-Djava.awt.headless=true !JVM_ARGS!
set TOOL_JAVA_CMD_QUOTED=!JAVA_CMD_QUOTED! !JVM_ARGS! -jar "!WLP_INSTALL_DIR!\bin\tools/ws-featureManager.jar"

@REM Execute the tool script or JAR.
if exist "!WLP_INSTALL_DIR!\lib\tools-internal/featureManager.bat" goto:script
...
:script
set JAVA_RC=
call "!WLP_INSTALL_DIR!\lib\tools-internal/featureManager" %*
if defined JAVA_RC (
  set RC=!JAVA_RC!
  call:javaCmdResult
)
